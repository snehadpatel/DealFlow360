"""Alembic environment, wired to the SQLModel metadata.

Importing ``app.models`` registers every table on ``SQLModel.metadata`` so that
``alembic revision --autogenerate`` sees the complete schema. The database URL

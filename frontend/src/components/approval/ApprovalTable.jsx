          ...
          <tbody className="divide-y divide-surface-border">
            {items.map((item) => (
              <tr key={item.id} onClick={() => onReview(item.id)} className="hover:bg-gray-50 transition-colors cursor-pointer">
                ...
                <td className="py-4 px-4 font-semibold text-brand-500">{item.quotation_id}</td>
                <td className="py-4 px-4 font-medium text-textPrimary">{item.customer_name}</td>
                <td className="py-4 px-4 text-xs">{item.sales_rep_name}</td>
                <td className="py-4 px-4 text-right font-bold text-textPrimary">{formatCurrency(item.total_value)}</td>

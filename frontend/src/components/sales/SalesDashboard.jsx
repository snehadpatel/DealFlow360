...
export default function SalesDashboard({ onAction }) {
  ...
  if (loading) {
    return (
      <div className="space-y-6 animate-pulse">
        <div className="h-24 bg-surfaceDark rounded-2xl" />
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[1, 2, 3, 4, 5, 6].map(i => <div key={i} className="h-28 bg-white border border-gray-200 rounded-2xl p-5" />)}
        </div>
      </div>
    );
  }
  ...
  const kpis = [
    ...
    { title: 'Avg Margin', count: `${data.avgMargin}%`, icon: Percent, color: 'text-brand-500', bg: 'bg-brand-50 border-brand-200', tab: 'dashboard' },
  ];

  return (
    <div className="space-y-6">
      <div className="bg-surfaceDark text-white p-6 rounded-2xl shadow-md flex flex-col sm:flex-row items-center justify-between gap-4 relative overflow-hidden">
        ...
        <button
          ...
          className="z-10 inline-flex items-center space-x-2 px-5 py-2.5 bg-brand-500 hover:bg-brand-600 text-white rounded-full text-sm font-semibold shadow-btn transition shrink-0"
        >
          ...
          return (
            ...
            >
              <div>
                ...
                <div className="text-3xl font-extrabold text-textPrimary mt-2 leading-none">{kpi.count}</div>
              </div>

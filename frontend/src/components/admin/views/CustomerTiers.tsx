...
import { fetchCustomersList, fetchDiscountRulesList } from "../../../api/adminApi";
...
export default function CustomerTiers() {
  ...
  async function loadTierData() {
    try {
      ...
      // Real per-tier customer counts from the DB.
      const counts: Record<string, number> = {};
      if (Array.isArray(customers)) {
        customers.forEach((c: any) => {
          const key = String(c.tier || "").toUpperCase();
          if (key) counts[key] = (counts[key] || 0) + 1;
        });
      }

      // Real per-tier discount ceilings from discount rules (tier-scoped).
      const ruleByTier: Record<string, number> = {};
      if (Array.isArray(rules)) {
        rules.forEach((r: any) => {
          if (r.tier && !r.category) ruleByTier[String(r.tier).toUpperCase()] = r.max_discount;
        });
      }
      ...
      setTiers((prev) =>
        prev.map((t) => {
          ...
          return {
            ...t,
            customers: counts[key] ?? t.customers,
            maxDiscount: ruleByTier[key] ?? t.maxDiscount,
          };
        })
      );
    } catch (e) {
      console.warn("Using initial tier data fallback", e);
    }
  }
